...
p2 = contourf(mach.model, mach.fitresult, X, y; plot_classification_loss=true, zoom=0, temp=temp, clim=nothing, loss_matrix=ones(2,2))
...
temp = 0.5
p1 = contourf(mach.model, mach.fitresult, X, y; plot_set_loss=true, zoom=-1, temp=temp, clim=nothing)
p2 = contourf(mach.model, mach.fitresult, X, y; plot_classification_loss=true, zoom=-1, temp=temp, clim=nothing, loss_matrix=ones(4,4))
plot(p1, p2, size=(800,320))
```
...
plt = data(df) * visual(BoxPlot) * 
    mapping(:generator, :value, row=:variable, col=:dataname, color=:generator)
draw(plt, axis=(xlabel="", xticksvisible=false, xticklabelsvisible=false, width=200, height=180))
```
