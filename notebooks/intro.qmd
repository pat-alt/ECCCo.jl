...
temp = 0.5
p1 = Plots.contourf(mach.model, mach.fitresult, X, y; plot_set_loss=true, zoom=-1, temp=temp, clim=nothing)
p2 = Plots.contourf(mach.model, mach.fitresult, X, y; plot_classification_loss=true, zoom=-1, temp=temp, clim=nothing, loss_matrix=ones(4,4))
Plots.plot(p1, p2, size=(800,320))
```
...
```{julia}
@chain bmk() begin
    @group_by(model, dataname)
    @select(model, dataname, ce)
    @mutate(performace = CounterfactualExplanations.model_evaluation(ce.M, ce.data))
    @summarize(model=unique(model), dataname=unique(dataname), performace=unique(performance))
    @ungroup
end
```
