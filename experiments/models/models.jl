...
function prepare_models(exper::Experiment; save_models::Bool = true)
    ...
    if !exper.use_pretrained
        if isnothing(exper.builder)
            if tuned_mlp_exists(exper) && exper.use_tuned
                ...
                best_mlp = Serialization.deserialize(
                    joinpath(tuned_model_path(exper), "$(exper.save_name)_best_mlp.jls"),
                )
                ...
                builder = default_builder(
                    ...
                    n_layers = exper.n_layers,
                    activation = exper.activation,
                )
            end
        else
        ...
        model_dict = train_models(
            ...
            train_parallel = exper.train_parallel,
            cov = exper.coverage,
        )
    else
        # Pre-trained models:
        if !(is_multi_processed(exper) && MPI.Comm_rank(exper.parallelizer.comm) != 0)
            ...
            model_dict = Serialization.deserialize(
                joinpath(pretrained_path(exper), "$(exper.save_name)_models.jls"),
            )
        else
    ...
    # Save models:
    local_models_exist =
        isfile(joinpath(DEFAULT_OUTPUT_PATH, "$(exper.save_name)_models.jls"))
    on_root_process =
    ...
    if save_models && on_root_process && !local_models_exist
        ...
        Serialization.serialize(
            joinpath(exper.output_path, "$(exper.save_name)_models.jls"),
            model_dict,
        )
    end
