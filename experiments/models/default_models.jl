...
"Outer constructor."
TuningBuilder(; n_hidden = 32, n_layers = 3, activation = Flux.swish) =
    TuningBuilder(n_hidden, n_layers, activation)
...
function default_builder(;
    n_hidden::Int = 16,
    n_layers::Int = 3,
    activation::Function = Flux.swish,
)
    builder =
        TuningBuilder(n_hidden = n_hidden, n_layers = n_layers, activation = activation)
    return builder
end
...
function default_models(;
    ...
    jem = JointEnergyClassifier(
        ...
        finaliser = finaliser,
        loss = loss,
        jem_training_params = (α = α, verbosity = verbosity),
