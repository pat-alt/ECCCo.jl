...
tab <- dt[
    ...
    .(
      val_group = mean(value),
      std_group = sd(value)
    ),
    .(dataname, generator, model, variable, run, source)
]
...
tab <- tab[
    ...
    .(
      value = ifelse(
        source != "vision",
        sprintf("%1.2f ± %1.2f", mean(val_group), sd(val_group)),
        sprintf("%1.3f ± %1.3f", mean(val_group), sd(val_group))
      ),
      val = mean(val_group),
      std = sd(val_group)
    ),
...
# Change model names to reduce column space
levels(tab_i$model)[match("MLP Ensemble",levels(tab_i$model))] <- "MLP"
levels(tab_i$model)[match("JEM Ensemble",levels(tab_i$model))] <- "JEM"
...
caption <- sprintf(
  "Results for %s dataset. No model ensembling was used. Formatting details are the same as in Table~\\ref{tab:results-tabular}. \\label{tab:results-%s} \\newline",
  chosen_source,
  chosen_source
)
file_name <- sprintf(
...
tab <- tab[,
    .(
      value =  sprintf("%1.2f ± %1.2f", mean(val_group), sd(val_group)),
      val = mean(val_group),
      std = sd(val_group)
    ),
    .(dataname, generator, model, variable, source)
