...
We perform benchmarks on eight different datasets from different domains. From the credit and finance domain we include three tabular datasets: Give Me Some Credit (\textit{GMSC})~\citep{kaggle2011give}, \textit{German Credit} and \textit{California Housing} (REFRENCES). All of these are commonly used in the related literature~\citep{karimi2020survey,altmeyer2023endogenous,pawelczyk2021carla}. Following~\citet{schut2021generating} we also include two image datasets: \textit{MNIST}~\citep{lecun1998mnist} and \textit{Fashion MNIST}~\citep{xiao2017fashion}. These are small image datasets but they allow us to easily visually gauge how plausible counteractuals are. (EXPLAIN WHY NO LARGER?) Finally, we also benchmark our generators on multiple synthetic datasets. Detailed descriptions and results for all datasets can be found in the appendix (POINTER). 

In the following we will focus on the most relevant results highlighted in Tables~\ref{tab:results-tabular} and~\ref{tab:results-vision}.  
...
Our key finding with respect to Research Question~\ref{rq:faithfulness} is as follows:

\begin{quote}
  We find strong empirical evidence that \textit{ECCCo} consistently achieves state-of-the-art faithfulness, as it was designed to do (Research Question~\ref{rq:faithfulness}).
\end{quote}

\subsection{Balancing Objectives}

With respect to balance the objectives of faithfulness and plausibility (Research Question~\ref{rq:plausibility}) we find the following:

\begin{quote}
  The empirical evidence presented here suggests that \textit{ECCCo} consistently achieves near state-of-the-art plausibility without sacrificing faithfulness (Research Question~\ref{rq:plausibility}). 
\end{quote}

\textit{ECCCo} achieves this result primarily by leveraging the model's generative property.
